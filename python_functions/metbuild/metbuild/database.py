class Database:
    ...
    def generate_file_list(self, service, param, start, end, storm, nowcast, multiple_forecasts):
        ...
        if service == "gfs-ncep":
            ...
            return self.generate_storm_file_list("hwrf", start, end, storm, nowcast, multiple_forecasts)
        elif service == "coamps-tc":
            return self.generate_storm_file_list("coamps_tc", start, end, storm, nowcast, multiple_forecasts)
        else:
    ...
    def generate_storm_file_list(self, data_type, start, end, storm, nowcast, multiple_forecasts):
        from datetime import timedelta
        if nowcast:
            return self.generate_storm_file_list_nowcast(data_type, start, end, storm)
        else:
            if multiple_forecasts:
                return self.generate_storm_file_list_multiple_forecasts(data_type, start, end, storm)
            else:
                return self.generate_storm_file_list_single_forecast(data_type, start, end, storm)
    ...
    def generate_storm_file_list_nowcast(self, data_type, start, end, storm):
        ...
        sql_tmptable = "create temporary table tmptbl1 select id,forecastcycle,forecasttime,filepath from "+data_type+" where stormname = '"+storm+"';"
